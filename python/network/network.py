...
class Network(list):
  ...
  def __init__(self, init_nodes=None):
    ...
    if init_nodes:
      ...
      self.extend(nodes)
      clusters = [Node(i, self) for i in range(cf.NB_NODES, cf.NB_NODES + cf.NB_CLUSTERS)]
  ...
  def simulate(self):
    ...
    for round_nb in range(0, cf.MAX_ROUNDS):
      ...
      if cf.TRACE_PACKETS_RECEIVED:
        tracer['packets_received'][2].append(self.get_nb_packets_received())
      if self.sleep_scheduler_class:
  ...
  def get_nb_packets_received(self):
    packets_received = 0
    for node in self:
      packets_received += node.amount_transmitted
