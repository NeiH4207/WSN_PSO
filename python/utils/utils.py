...
def calculate_vector_point(x1, y1, x2, y2):
  """Calculate the vector between two points."""
  return (x2 - x1, y2 - y1)

def calculate_vector(node1, node2):
  """Calculate the vector between two nodes."""
  x1 = node1.pos_x
  y1 = node1.pos_y
  x2 = node2.pos_x
  y2 = node2.pos_y
  return calculate_vector_point(x1, y1, x2, y2)
...
def plot_traces(traces):
  first_tracer = list(traces.values())[0]
  ...
  for scenario, tracer in traces.items():
    ...
    plt.savefig(cf.RESULTS_PATH + time.strftime("%Y-%m-%d_%H:%M:%S") + '_' + scenario + '.png')
